...
from cmd_args_sst import args, use_lstm
from tensorboard_logger import log_value
import torch.nn.functional as F
import numpy as np
from torch.nn.utils import clip_grad_norm_
from data.sst_dataset_lstm import get_sst_train_and_val_loader,get_SST_model_and_loss_criterion
import json
...
def train_others(args, model, loader, optimizer, criterion, global_iter, epoch, logpath):
    ...
    #loss_lst = TDigest()
    loss_lst = []
    for i, (data, length, target, target_gt, index) in enumerate(loader):
        global_iter += 1
...
def validate(args, model, loader, criterion, epoch, logpath, mode='val'):
    ...
    t0 = time.time()
    with torch.no_grad():
        for i, (data, length, target, target_gt, _) in enumerate(loader):
            data, target = data.to(args.device), target.to(args.device)
