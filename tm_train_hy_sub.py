...
from common.utils import hook_fn_random_walk,hook_fn_moutput,hook_fn_parameter
...
def main(params):
    ...
    # # ADD
    # args.exp_name = os.path.join(args.exp_name, f'nr{args.noise_rate:.1f}')
    ...
    if 'STGN' in args.exp_name:
        ...
        if 'GCE' in args.exp_name:
            args.q = params['q']
            args.exp_name = os.path.join(args.exp_name, f'sigma{args.sigma}_times{args.times}_q{args.q}')
        # 参数添加到路径中
        else:
            args.exp_name = os.path.join(args.exp_name, f'sigma{args.sigma}_times{args.times}')

    #args.noise_rate = params['noise_rate']

    elif 'GCE' in args.exp_name:
        args.q = params['q']
        # 参数添加到路径中
        args.exp_name = os.path.join(args.exp_name, f'q{args.q}')

    if ('SLN' in args.exp_name) or ('GNMP' in args.exp_name) or ('GNMO' in args.exp_name):
        args.sigma = params['sigma']
        
        # 参数添加到路径中
        args.exp_name = os.path.join(args.exp_name, f'sigma{args.sigma}')
    ...
    # GNMP 对网络中的参数施加扰动，bert+embedding参数非常多
